...
# Generated by ewave_batch: the full command for this run, replayable by hand.
# Editing this file does not change the batch state (batch.json is the authority).
...
class VerifyRunOutputs(unittest.TestCase):
    ...
    def test_zero_byte_sparam_is_rejected(self) -> None:
        ...
        self.assertTrue(any("is 0 bytes" in reason for reason in report.reasons), report.reasons)
    ...
    def test_port_count_mismatch_is_rejected(self) -> None:
        ...
        self.assertTrue(
            any("port count mismatch" in reason for reason in report.reasons), report.reasons
        )
    ...
    def test_explicit_expected_port_count_wins(self) -> None:
        ...
        self.assertFalse(report.ok)
        self.assertTrue(any("port count mismatch" in reason for reason in report.reasons))
    ...
    def test_mixed_port_counts_in_one_run_are_rejected(self) -> None:
        ...
        self.assertFalse(report.ok)
        self.assertTrue(
            any("port counts disagree" in reason for reason in report.reasons), report.reasons
        )
...
class ArchiveRun(unittest.TestCase):
    ...
    def test_counts_kept_and_removed_negative(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                any("verify first, delete after" in err for err in report.errors), report.errors
            )
            self.assertEqual(
...
class StateRoundTrip(unittest.TestCase):
    ...
    def test_run_groups_survive_the_round_trip(self) -> None:
        """`BatchState.groups` 和 `Run.group` 必须原样活过 `batch.json` 的一来一回。

        这条 2026-08-19 之前是 `expectedFailure`（写下来的已知缺口），当天补上了实现、
        装饰器随之删掉。缺口的后果值得留在这里：resume 一个带组的批次时 run 本身认得出来
        （`run_id` 没变），但「这个 run 出自哪个组」丢了 ⇒ Runs 表的 Group 列对**每一条**
        run 都显示 `base`，`GuiState.group_of()` 会说谎，而界面上一点提示都没有。

        两个字段都带默认值 ⇒ **双向兼容** ⇒ `SCHEMA_VERSION` 不用 +1：
        老的 batch.json 没有 `group` 键，读回来落 `base`（= 整批只有 base 组，
        与加组之前逐字相同）。下面 `test_old_batch_json_without_groups_still_reads` 守这条。
        """
        state = model.BatchState(
            batch_name="grouped",
            batch_dir="/tmp/grouped",
            runs=[
                model.Run(run_id="d/eqI-on/typical_55_0", design_key="d"),
                model.Run(run_id="d/eqI-off/typical_55_0", design_key="d", group="eqcur-off"),
            ],
            groups=[
                model.RunGroup(
                    name="eqcur-off",
                    axis_overrides={"equalCurrent": ("off",)},
                    label="one-off",
                )
            ],
        )
        back = layout.state_from_dict(layout.state_to_dict(state))
        self.assertEqual([g.name for g in back.groups], ["eqcur-off"])
        self.assertEqual(
            [{k: tuple(v) for k, v in g.axis_overrides.items()} for g in back.groups],
            [{"equalCurrent": ("off",)}],
        )
        self.assertEqual([run.group for run in back.runs], ["base", "eqcur-off"])
        self.assertEqual([g.label for g in back.groups], ["one-off"])

    def test_old_batch_json_without_groups_still_reads(self) -> None:
        """向后兼容：加组之前写出来的 batch.json 没有 `groups` / `group` 两个键。

        读回来必须是"整批只有 base"，而不是报错、也不是空组名 —— 空组名会让 Runs 表的
        Group 列变成一列空白，比说错更难看出是怎么回事。
        """
        old = layout.state_to_dict(
            model.BatchState(
                batch_name="old",
                batch_dir="/tmp/old",
                runs=[model.Run(run_id="d/base/typical_55_0", design_key="d")],
            )
        )
        old.pop("groups")
        old["runs"][0].pop("group")
        back = layout.state_from_dict(old)
        self.assertEqual(list(back.groups), [])
        self.assertEqual([run.group for run in back.runs], ["base"])
...
class RunsCsv(unittest.TestCase):
    ...
    def test_header_and_row(self) -> None:
        ...
        self.assertEqual(
            ...
            "eqI-off,typical_-40_0,failed,1234,,,12.5,1024.5,4,,sparam/demo.s4p,崩了但 exit=0,base",
        )
...
class SetRunAsCurrent(unittest.TestCase):
    ...
    def test_copies_with_backup_and_log(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(line.startswith("backup ") for line in actions), actions)
