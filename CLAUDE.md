...
1b. **本项目自己的代码将来要上 public GitHub（用户 2026-08-17 决定）→ 源码里零红区标识符。**
   ...
   - 工号/主机名/项目代号/workarea 路径/library/cell/view/端口名/PDK 版本串/ptxt 路径/
     工具绝对路径 —— **一律不进源码**。
   - ⚠️ **唯一的例外：Donau 账号与队列**（用户 2026-08-28 明确批准，原话「我觉得问题不大，
     就是服务器名字这些而已；我让你改默认值」）。落点只有两行：`gui.state`
     的 `DEFAULT_SUBMIT_ACCOUNT` / `DEFAULT_SUBMIT_QUEUE`，行尾带 `redzone-allow` 注释
     给闸门看。起因是 08-28 在别人机器上部署实测的卡手：占位符默认值意味着每台新机器
     都得先配一次 `site.local.sh`，而那一步没人猜得到该填什么。
     **例外的边界就是那两行，不许外推**——上面那一串（尤其 library/cell/view/端口名/ptxt）
     一个字没变，它们描述的是设计和版图，不是一台调度器的名字。要再加新的站点值进源码，
     先问用户，别拿这条当先例自行推广。
