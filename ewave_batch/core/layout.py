...
def _assert_outside_spine(path: str, what: str) -> None:
    ...
    for part in _posix(path).split("/"):
        if part == SPINE_DIRNAME:
            raise StateError(
                f"{what}落在 {SPINE_DIRNAME}/ 里面了：{path}\n"
                f"  那是官方 GUI 的地盘（设计师的 spine），本工具只读它。\n"
                f"  把 batch_root 换到别处，例如 <workarea>/ewave_batches。\n"
                f"  唯一例外是显式触发的「把这个 run 设为当前」（set_run_as_current）。"
            )
...
def _validate_component(name: str, what: str) -> str:
    ...
    text = str(name).strip()
    if not text:
        raise StateError(f"{what}是空的 —— 拼不出目录名")
    if "/" in text or "\\" in text:
        raise StateError(f"{what}里有路径分隔符: {name!r} —— 它只能是一层目录名")
    if text in (".", ".."):
        raise StateError(f"{what}是 {name!r} —— 那会把落点顶到批次外面去")
    return text
...
def _design_dir_name(design: Design, run: Run) -> str:
    ...
    **故意不 import `core.matrix`**：这里只需要一个稳定的目录名，而 `Run.design_key`
    ...
    return _validate_component(fallback, "design 目录名（library_cell_view 兜底）")
...
def compute_run_paths(batch_dir: str, design: Design, run: Run) -> RunPaths:
    ...
    **纯函数，一个目录都不建**（`ensure_run_dirs` 才建）。路径全用 `/` 拼，
    ...
    if not root:
        raise StateError("batch_dir 是空的 —— 没有落点就没有布局")
...
def write_cmd_sh(paths: RunPaths, plan: CommandPlan, *, dry_run: bool = False) -> str:
    ...
    * `dry_run=True` 时只返回路径、不写文件。
    ...
    if not plan.argv:
        raise StateError(f"CommandPlan 没有 argv —— 写不出 cmd.sh: {target}")

    lines: list[str] = ["#!/bin/sh"]
    lines.append("# 由 ewave_batch 生成：这个 run 的完整命令，可单独手工重跑。")
    lines.append("# 改这个文件不会改批次状态（batch.json 才是权威）。")
    if plan.run_id:
...
def verify_run_outputs(
    ...
    *,
    ...
    if not os.path.isdir(ewave_dir):
        return VerifyReport(
            ok=False,
            reasons=(f"输出目录不存在: {ewave_dir}（eWave 那层 <corner>_<temp> 根本没建起来）",),
            ports=ports,
        )
    ...
    if not sparam_names:
        return VerifyReport(
            ok=False,
            reasons=(f"{ewave_dir} 里没有 .sNp 产物（有 {len(names)} 个别的文件）",),
            ports=ports,
        )
    ...
    for name in sparam_names:
        ...
        try:
            ...
            reasons.append(f"读不到 {name} 的大小: {exc}")
            continue
        files.append(full)
        total_bytes += size
        if size == 0:
            # ★ 这不是假想：MVP 实测过 eWave 崩了也留 0 字节文件、还报 "done"（BRIEF §10）。
            reasons.append(f"{name} 是 0 字节 —— eWave 报 done 也不算数（BRIEF §10 实测）")
        counts.append(port_count_from_suffix(name))

    distinct = sorted({c for c in counts if c is not None})
    if len(distinct) > 1:
        reasons.append(f"同一个 run 里的 .sNp 端口数不一致: {distinct} —— 产物是混的，别往下比")
    ...
    if want is not None:
        if port_count is None:
            reasons.append(f"认不出产物的端口数（文件: {', '.join(sparam_names)}），期望 {want}")
        elif port_count != want:
            # `--all` 的代价：pin 集合一变所有端口编号平移，而且**静默**（BRIEF §5）。
            reasons.append(f"端口数不符: 产物是 {port_count} 端口，期望 {want} 端口")
...
def archive_run(
    ...
    * **删除只在 `paths.ewave_dir` 里发生**，别的地方一个文件都不许删。
    ...
    if not verdict.ok:
        errors.append(
            "产物验收没过 —— 一个文件都不删（先验后删）: " + "; ".join(verdict.reasons)
        )
        return ArchiveReport(
            kept=tuple(files),
            removed=(),
    ...
    if not matched:
        ...
        errors.append(
            f"keep 模式 {list(patterns)} 一个文件都没命中 —— 一个文件都不删"
            "（照删会把这个 run 的全部产物一起删掉，那是不可逆的）"
        )
        return ArchiveReport(
            kept=tuple(files),
            removed=(),
    ...
    for name in sorted(matched):
        ...
        try:
            ...
            if os.path.getsize(dst) != os.path.getsize(src):
                raise OSError("副本大小和原件对不上")
        except OSError as exc:
            copy_failed = True
            errors.append(f"收进扁平区失败 {name} -> {dst}: {exc}")

    if copy_failed:
        errors.append("有副本没落成 —— 一个文件都不删（先验后删）")
        return ArchiveReport(
            kept=tuple(files),
            removed=(),
            missing=missing,
    ...
    for name in files:
        ...
        if not _inside(full, ewave_dir):  # pragma: no cover - 防御：符号链接指到外面去
            errors.append(f"跳过 {name}：它指到 {ewave_dir} 外面去了，删除只在本 run 目录内发生")
            kept.append(name)
            continue
        try:
            size = os.path.getsize(full)
    ...
    **按 design 分组比**：不同 design 端口数本来就不同（官方样本一个 17 端口一个 16 端口），
    ...
    for design_key in sorted(by_design):
        ...
        for run in runs[1:]:
            ...
            if ports == ref_ports:
                continue
            detail = f"{len(ports)} 个 vs {len(ref_ports)} 个"
            for index, (got, want) in enumerate(zip(ports, ref_ports)):
                if got != want:
                    detail = f"第 {index} 位: {got!r} vs {want!r}"
                    break
            problems.append(
                f"design {design_key}: run {run.run_id} 的端口列表与 {reference.run_id} 不一致"
                f"（{detail}）—— 端口编号平移会让 .sNp 和 nport 静默错位，别继续比数字"
            )
    return problems
...
def _enum_from(cls, value: object, what: str):
    try:
        ...
        raise StateError(f"{what}: 不认识的取值 {value!r}（batch.json 是别的工具写的？）") from exc


def _req(data: Mapping[str, object], key: str, what: str) -> object:
    if key not in data:
        raise StateError(f"{what} 缺字段 {key!r}")
    return data[key]
...
def _job_from_dict(data: object) -> Job | None:
    if data is None:
        return None
    if not isinstance(data, Mapping):
        raise StateError(f"Job 不是一个对象: {data!r}")
...
def _log_facts_from_dict(data: object) -> LogFacts | None:
    if data is None:
        return None
    if not isinstance(data, Mapping):
        raise StateError(f"LogFacts 不是一个对象: {data!r}")
    return LogFacts(
...
def state_from_dict(data: Mapping[str, object]) -> BatchState:
    ...
    if not isinstance(data, Mapping):
        raise StateError(f"batch.json 顶层不是一个对象: {type(data).__name__}")
    raw_version = data.get("schema_version", SCHEMA_VERSION)
    try:
        version = int(raw_version)
    except (TypeError, ValueError) as exc:
        raise StateError(f"schema_version 不是整数: {raw_version!r}") from exc
    if version > SCHEMA_VERSION:
        raise StateError(
            f"batch.json 的 schema_version={version}，本工具只认到 {SCHEMA_VERSION} —— "
            "拒绝而不是猜（猜错了会把别人的批次改坏）。请升级工具。"
        )
    ...
    try:
        ...
        raise StateError(f"batch.json 结构不对: {exc}") from exc
...
def read_batch_state(path: str) -> BatchState:
    ...
    try:
        ...
        raise StateError(f"没有 {target} —— 这个目录不是一个批次（resume 只认 batch.json）") from exc
    except OSError as exc:
        raise StateError(f"读不了 {target}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise StateError(f"{target} 不是合法 JSON（写到一半断电了？）: {exc}") from exc
...
def write_batch_state(path: str, state: BatchState) -> None:
    ...
    try:
        ...
        raise StateError(f"写不了 {target}: {exc}") from exc
...
def _primary_sparam(ewave_dir: str, design: Design) -> str:
    ...
    if not os.path.isdir(ewave_dir):
        raise StateError(f"输出目录不存在: {ewave_dir}")
    candidates = [
        n
    ...
    if not candidates:
        raise StateError(f"{ewave_dir} 里没有 .sNp —— 这个 run 没有可以设为当前的产物")
    non_empty = [n for n in candidates if os.path.getsize(_join(ewave_dir, n)) > 0]
    if not non_empty:
        raise StateError(
            f"{ewave_dir} 里的 .sNp 全是 0 字节 —— 拒绝把空文件盖到设计师的 spine 上（BRIEF §10 实测）"
        )
    ...
    *,
    ...
    if not target or not os.path.isdir(target):
        raise StateError(
            ...
            "  这一步要写进设计师的 spine —— 目录都不在，就更不该替他建。"
        )
    ...
    if os.path.exists(dest):
        ...
        actions.append(f"备份 {dest} -> {backup}")
        if not dry_run:
            shutil.copy2(dest, backup)

    actions.append(f"复制 {source} -> {dest}")
    if not dry_run:
        shutil.copy2(source, dest)

    log_path = _join(paths.logs_dir, SET_CURRENT_LOG_NAME)
    actions.append(f"记日志 {log_path}")
    if not dry_run:
