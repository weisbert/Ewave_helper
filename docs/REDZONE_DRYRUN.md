...
--batch-root DIR    落点的根，只用来算路径、**不会建目录**（默认 <install>/ewave_batches）
...
2. 所有「真跑时会写这里」的落点都经过一道守卫：落点在 `ewave_simulation/` 里面
   ...
   任何命令**之前**拒绝的。
3. 落点默认是 `<install>/ewave_batches`（**不是**相对当前目录 —— 那样落点会跟着 cwd 漂；也**不是** `~/`，红区 `$HOME` 有配额，2026-08-24 撞过）。安装目录里那份由 `deploy.sh` 的 `PRESERVE` + `looks_like_batch_data` 保住。而且本命令**连这个目录都不会建**。
